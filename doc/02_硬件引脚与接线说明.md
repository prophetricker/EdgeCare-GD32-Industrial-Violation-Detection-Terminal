...
本文档记录 `GD32H759I-START` 初赛演示版本的当前实际接线。

逐线核对时优先看 [`03_快速接线表.md`](03_快速接线表.md)。
...
| GD32VW553 voice board | `UART2_RX / PA7` | `PA2 / USART1_TX` | H7 -> VW553 | H7 sends `DANGER\n` |
| GD32VW553 voice board | `UART2_TX / PA6` | `PA3 / USART1_RX` | VW553 -> H7 | Optional response: `OK\r\n` / `ERR\r\n` |
| GD32VW553 voice board | `GND` | `GND` | common | Must share ground |
| GD32VW553 voice board | `VCC` | board/module-rated power | power | Confirm the actual board power input before wiring |
...
## GD32VW553 Voice Warning UART

The current formal H759-to-VW553 path is a voice warning command, not the older
placeholder JSON upload log. The H759 sends one command on the alarm rising edge:
...
```text
115200 baud, 8N1, 3.3 V logic, common GND
```

Current wiring:

| H759 side | VW553 side | Notes |
| --- | --- | --- |
| `PA2 / USART1_TX` | `PA7 / UART2_RX` | Required; H759 transmits `DANGER\n` |
| `PA3 / USART1_RX` | `PA6 / UART2_TX` | Optional but wired; VW553 can reply |
| `GND` | `GND` | Required common ground |

H759 startup log should include:

```text
voice_uart: target=GD32VW553 USART1_TX=PA2 USART1_RX=PA3 baud=115200 command=DANGER\n
```

When the visual alarm changes from inactive to active, H759 sends the command
once and enforces a `3000 ms` cooldown:

```text
voice_probe: event=danger_cmd target=vw553 uart=USART1 tx=PA2 rx=PA3 command=DANGER cooldown_ms=3000
```

VW553 firmware note: the teammate voice branch originally used `PC8/BOOT0` as a
local active-low simulator. For the formal UART path, VW553 must run a firmware
that listens on `UART2 PA7/RX` and calls `voice_prompt_player_trigger()` when it
receives `DANGER`.

Remaining optional wiring:
