...
- 可演示功能：GD32H759 Keil 工程可编译；LD2410 触发、报警灯 GPIO、USART0 日志、OV5640 QVGA YUV422 采集、`96x96` 灰度预处理和 placeholder 推理接口已打通。
...
- 当前重构状态：`board_config`、`bsp_alarm`、`bsp_radar_ld2410`、`edgecare_log`、`edgecare_preprocess`、`edgecare_infer` 已抽出并编译通过。
- 编译命令：
...
- [ ] 下一轮处理摄像头模块边界：先只抽 SCCB/OV5640/DCI/DMA 相关代码到 `bsp_camera_ov5640`，保持启动 probe 日志不变。
...
- [ ] 下一轮重构先从 `board_config.h`、`bsp_alarm`、`bsp_radar_ld2410` 开始，保持每步 Keil 编译通过。
