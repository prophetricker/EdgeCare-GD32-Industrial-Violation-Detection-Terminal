...
- 2026-06-08：新增根目录 `README.md` 作为 GitHub 协作入口，说明项目目标、当前能力、目录结构、构建依赖、文档入口和禁止上传内容；计划把必要源码与轻量文档推送到 `prophetricker/EdgeCare-GD32-Industrial-Violation-Detection-Terminal`。
...
下一步用户动作：先不要继续采集训练集。硬件侧用 Keil 烧录当前 workspace 最新构建，关闭 Serial Monitor 后运行 `py .\tools\collect_gray96_serial.py --label empty --kind byte_plane --count 4 --variant any --port COM8 --max-wait-sec 90`，打开脚本监听后按 RESET，保存四张 byte-plane 诊断图，并回传 `camera_init:` 到 `camera_capture_best:` 的串口日志和 `.bmp` 视觉效果；这轮期望看到的是规则 color bar，不是两瓶水真实画面。若仍全是水平纹，继续排查 `D0-D7` 接线/位序、`PCLK` 采样边沿和 DCI 同步窗口，不进入模型训练。
...
- GitHub 上传策略：先推送当前已验证的 `feature/firmware-module-refactor`，不直接覆盖 `main`；数据集、模型、PDF/压缩包、完整 SDK 和 Keil 产物继续留在本地。
