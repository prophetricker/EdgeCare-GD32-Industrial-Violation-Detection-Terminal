...
下一步用户动作：先不要运行 byte-plane 采集。断电后拆掉相机 `PCLK/HREF/SYNC/D0-D7`，只保留 `3V3/GND/SCL->PB10/SDA->PB11/RES->PD0/PWON->PD1`；上电后用万用表测相机模块端 `3V3-GND`、`RES-GND`、`PWON-GND`、`SCL-GND`、`SDA-GND`，再复位并确认启动日志恢复 `camera_id: ov5640_regs[0x300A,0x300B]=0x56 0x40`。ID 恢复后再分两步接回 `PCLK/HREF/SYNC` 和 `D0-D7`。
