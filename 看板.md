...
下一步用户动作：保持当前已恢复的 `3V3/GND/SCL/SDA/RES/PWON/PCLK` 不动，断电后只重查两根同步线。相机下排从靠近 `3V3` 一侧依次是 `3V3 -> SYNC -> HREF -> RES`，确认 `SYNC -> PB7/JP11-25`、`HREF -> PA4/JP8-29`；用万用表通断档检查端到端导通，并确认二者不与 `3V3/GND/RES` 或相邻针短路。重新上电复位后，目标日志是 `href_edges` 达到数千以上且 `sync_edges` 非零；在此之前忽略后续 DMA/data-order 扫描结果。
