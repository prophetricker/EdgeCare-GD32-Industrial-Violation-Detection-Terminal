...
def test_training_exports_configured_grid_to_header_and_metrics() -> None:
    tmp = Path(tempfile.mkdtemp(prefix="edgecare_train_grid8_"))
    try:
        root = tmp / "raw_gray96"
        out = tmp / "model"
        for i, value in enumerate([8, 16, 24, 32]):
        ...
        header = (out / "edgecare_model_baseline.h").read_text(encoding="utf-8")
        metrics = (out / "baseline_metrics.json").read_text(encoding="utf-8")
        assert "#define EDGECARE_BASELINE_GRID 8" in header
        assert "#define EDGECARE_BASELINE_FEATURE_COUNT 69" in header
        assert '"grid": 8' in metrics
    finally:
        shutil.rmtree(tmp)
...
if __name__ == "__main__":
    ...
    test_training_exports_configured_grid_to_header_and_metrics()
